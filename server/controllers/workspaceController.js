...
export const addMember = async (res , req) => {
   ...
   try{
    ...
    // if user doesnt exist with this email id 
    if(!user){
  ...
  // if user is avialable we have to check for the role and worekdapsce
  
   if(!workspaceId || !role){
        return res.status(400).json({message: "UMissing requires parameters"});
     }
